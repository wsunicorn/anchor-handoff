 ...
 * → rerank 20→6 → generate (stream) → ghi messages/usage_costs/cache.
 * Câu trả lời ở đây là **thô** — app chỉ hiển thị sau verify() (G3, CLAUDE.md quy tắc 2).
 */
  ...
  try {
    ...
    (async () => {
      try {
        ...
        if (cached) {
          const a = cached.answer as { text: string; citations: Record<string, CitationSource>; insufficient: boolean; nearest_page: number | null };
        ...
        await admin.from('answer_cache').upsert({
          ...
          document_id,
          lang,
          answer: { text: result.text, citations, insufficient, nearest_page: nearestPage, model: ANSWER_MODEL },
        });
