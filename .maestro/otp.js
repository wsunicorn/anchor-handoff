...
// Lấy mã OTP 6 số mới nhất gửi tới EMAIL từ Mailpit của Supabase local (cổng 54314).
const list = JSON.parse(http.get('http://127.0.0.1:54314/api/v1/messages?limit=20').body);
const msg = (list.messages || []).find((m) => (m.To || []).some((t) => t.Address === EMAIL));
if (!msg) throw new Error('Mailpit: chưa có email cho ' + EMAIL);
const full = JSON.parse(http.get('http://127.0.0.1:54314/api/v1/message/' + msg.ID).body);
