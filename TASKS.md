...
> **Cổng hiện tại: G1** — cập nhật dòng này ở cuối mỗi phiên. (G0 đóng 2026-09-13.)
...
- [ ] G1.10 Giới hạn nạp: 1 tài liệu, tối đa 80 trang cho người chưa trả tiền (kiểm ở server, không kiểm ở client) — *server xong (403 `document_limit`/`page_limit`); còn hiển thị lỗi ở app G1.6*
...
| 5 | Cần **Gemini API key** (Google AI Studio, free tier đủ dev): https://aistudio.google.com/apikey → *Create API key* → dán vào `services/ingest/.env` dòng `LLM_PROVIDER_API_KEY=` và đổi `EMBEDDING_PROVIDER=gemini` (hoặc gửi tôi key). Cùng key này dùng cho Edge Function ở G2 (`supabase secrets set`). | G1.5 chất lượng thật, G2 | Chờ |
